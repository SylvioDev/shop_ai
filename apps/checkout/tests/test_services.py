...
class TestCheckoutService:
    def test_cart_validation_empty_cart(self):
        ...
        with pytest.raises(EmptyCartError) as exc_info:
            invalid_cart = container.checkout_service.cart_validation(cart)
    ...
    def test_cart_validation_valid_cart(self, cart_data):
        valid_cart = container.checkout_service.cart_validation(cart_data)
    ...
    def test_handle_order_status(self, cart_data, payment_status, expected_output):
        ...
        updated_order = container.checkout_service.handle_order_status(order.order_number, payment_status=payment_status)
        updated_order.status == expected_output
        
    def test_order_creation(self, cart_data):
        user = checkout_user('Nina', '80th Street')
        order = container.checkout_service.order_creation(cart=cart_data, user=user)
    ...
    def test_payment_creation(self, cart_data):
        ...
        order = container.checkout_service.order_creation(cart=cart_data, user=user)
        payment = container.checkout_service.payment_creation(order, 'stripe')
    ...
    def test_add_order_items_success(self, cart_data):
        user = checkout_user('Rams', '48th Street Elizabeth')
        order = container.checkout_service.order_creation(cart=cart_data, user=user)
        container.checkout_service.add_order_items(order, cart_data.cart)
    ...
    def test_add_order_items_failure(self, cart_data):
        with pytest.raises(AttributeError) as exc_info:
            order_items = container.checkout_service.add_order_items("qsdqsdqsd", cart_data.cart)
    ...
    def test_handle_success_payment_status(self, mocker, sku, product):
        ...
        order = container.checkout_service.order_creation(cart=cart, user=user)
        payment = container.checkout_service.payment_creation(order, 'stripe')    
        ...
        new_order = container.checkout_service.handle_success_payment_status(
    ...
    def test_handle_failure_payment_status(self, mocker, sku, variant_product, variant_sku):
        ...
        updated_order = container.checkout_service.handle_failure_payment_status(
            order_id=order.order_number,
            user_id=user,
    ...
    def test_handle_webhook_fallback(self, mocker, sku, product):
        ...
        new_order = container.checkout_service.handle_webhook_fallback(
            session_id=session_id,
            order_id=order.order_number,
            user_id=user
        )
